...
describe('util', function () {
    describe('hydrateRequest', function () {
        it('should take a component request and return a hydrated component request', function () {
            ...
            // assert
            expect(hydratedRequest).toEqual({ 'className': 'SettingsViewComponent', 'dirToRead': path_1.join(templateDir, 'component'), 'dirToWrite': path_1.join(componentsDir, 'settings-view'), 'fileName': 'settings-view', 'includeNgModule': true, 'includeSpec': true, 'name': 'settings view', 'type': 'component' });
        ...
        it('should take a page request and return a hydrated page request', function () {
            ...
            var hydratedRequest = util.hydrateRequest(context, request);
            // assert
            expect(hydratedRequest).toEqual({ 'className': 'SettingsViewPage', 'dirToRead': path_1.join(templateDir, 'page'), 'dirToWrite': path_1.join(pagesDir, 'settings-view'), 'fileName': 'settings-view', 'includeNgModule': true, 'includeSpec': true, 'name': 'settings view', 'type': 'page' });
    ...
    describe('getNgModules', function () {
        ...
        it('should return a list of glob results for components', function () {
            ...
            expect(globAllSpy).toHaveBeenCalledWith([path_1.join(componentsDir, '**', '*.module.ts')]);
        });
        it('should return a list of glob results for pages and components', function () {
            var globAllSpy = spyOn(globUtils, globUtils.globAll.name);
            spyOn(helpers, helpers.getStringPropertyValue.name).and.returnValue('.module.ts');
            util.getNgModules(context, ['page', 'component']);
            expect(globAllSpy).toHaveBeenCalledWith([path_1.join(pagesDir, '**', '*.module.ts'), path_1.join(componentsDir, '**', '*.module.ts')]);
        });
    });
    describe('tabsModuleManipulation', function () {
        ...
        it('should return a succesful promise', function () {
            ...
            spyOn(fs, 'readdirSync').and.returnValue([
                path_1.join(process.cwd(), 'path', 'to', 'nowhere'),
                path_1.join(process.cwd(), 'path', 'to', 'somewhere'),
            ]);
            spyOn(helpers, helpers.writeFileAsync.name).and.returnValue(Promise.resolve());
        ...
        it('should throw when files are not written succesfully', function () {
            var knownErrorMsg = "ENOENT: no such file or directory, open 'undefined/settings-view.module.ts'";
    ...
    describe('nonPageFileManipulation', function () {
        ...
        it('should throw when files are not written succesfully', function () {
            var knownErrorMsg = "ENOENT: no such file or directory, open 'src/pages/cool-tab-one/cool-tab-one.module.ts'";
            spyOn(helpers, helpers.writeFileAsync.name).and.returnValue(Promise.reject(new Error(knownErrorMsg)));
            var promise = util.nonPageFileManipulation(context, 'coolStuff', path_1.join('src', 'pages', 'cool-tab-one', 'cool-tab-one.module.ts'), 'pipe');
            return promise.then(function () {
                throw new Error('should never happen');
            }).catch(function (err) {
                expect(err.message).toEqual(knownErrorMsg);
