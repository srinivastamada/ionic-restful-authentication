...
function processPageRequest(context, name) {
    var hydratedRequest = util_1.hydrateRequest(context, { type: 'page', name: name });
...
function processTabsRequest(context, name, tabs) {
    var tabHydratedRequests = tabs.map(function (tab) { return util_1.hydrateRequest(context, { type: 'page', name: tab }); });
    var hydratedRequest = util_1.hydrateTabRequest(context, { type: 'tabs', name: name, tabs: tabHydratedRequests });
    return util_1.generateTemplates(context, hydratedRequest).then(function () {
